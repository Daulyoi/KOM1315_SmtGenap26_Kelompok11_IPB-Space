...
...
  ...
  /**
   * Get digital permit signature and verification URL
   * @param {string|number} bookingId 
   * @returns {Promise<any>}
   */
  getBookingPermit: async (bookingId) => {
    return await apiClient.get(`/bookings/${bookingId}/permit`);
  },

  /**
   * Publicly verify a cryptographic booking permit token
   * @param {string} token 
   * @returns {Promise<any>}
   */
  verifyBookingPermit: async (token) => {
    return await apiClient.get(`/bookings/verify/${token}`);
  }
};
