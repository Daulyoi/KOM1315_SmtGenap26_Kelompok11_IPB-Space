...
@router.get("/{booking_id}/permit", response_model=HTTPResponse)
async def get_booking_permit(
    booking_id: int,
    service: BookingService = Depends(get_booking_service),
    current_user: UserResponse = Depends(get_current_user)
) -> HTTPResponse:
    """
    Retrieve digital permit details and verification URL for an approved booking.
    """
    booking = await service.get_booking_by_id(booking_id)
    
    # Authorization: Owner, Admin, or Facility Manager
    is_owner = booking.user_id == current_user.id
    is_staff = current_user.role in [UserRoles.ADMIN.value, UserRoles.FACILITY_MANAGER.value]
    
    if not (is_owner or is_staff):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Kamu tidak memiliki izin untuk melihat permit booking ini"
        )
        
    if booking.status != "approved":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Permit digital hanya tersedia untuk booking dengan status 'approved'"
        )
        
    if not booking.digital_signature:
        # Re-generate signature if it is approved but lacks one (fallback for pre-existing approved bookings)
        try:
            from app.core.security_sign import CryptographicSigner
            from app.core.config import settings
            validator = booking.validated_by or "System"
            payload = {
                "bid": booking.id,
                "fac": booking.facility.name if booking.facility else "Facility",
                "usr": booking.user.fullname if booking.user else "Civitas",
                "dat": booking.date_of_booking.strftime("%Y-%m-%d"),
                "tim": f"{booking.start_time.strftime('%H:%M')}-{booking.end_time.strftime('%H:%M')}",
                "val": validator
            }
            signer = CryptographicSigner(private_key_hex=settings.SIGNING_PRIVATE_KEY)
            signature_token = signer.sign_payload(payload)
            await service.booking_repository.update(booking.id, {"digital_signature": signature_token})
            booking.digital_signature = signature_token
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Gagal membuat digital signature: {str(e)}"
            )
            
    from app.core.config import settings
    verification_url = f"{settings.BASE_URL}/bookings/verify/{booking.digital_signature}"
...
@router.get("/verify/{token}", response_model=HTTPResponse)
async def verify_booking_permit(
    token: str
) -> HTTPResponse:
    """
    Public endpoint to verify a cryptographic booking permit.
    """
    try:
        from app.core.security_sign import CryptographicSigner
        from app.core.config import settings
        
        signer = CryptographicSigner(public_key_hex=settings.SIGNING_PUBLIC_KEY)
        verified_payload = signer.verify_token(token)
        ...
        return HTTPResponse(
            ...
            data={
                ...
                "details": {
                    "booking_id": verified_payload.get("bid"),
                    "facility_name": verified_payload.get("fac"),
                    "user_fullname": verified_payload.get("usr"),
                    "date_of_booking": verified_payload.get("dat"),
                    "time_range": verified_payload.get("tim"),
                    "validated_by": verified_payload.get("val"),
