...
class BookingService:
    ...
    async def update_booking_status(self, booking_id: int, new_status: str, reason: str | None = None, validated_by: str | None = None):
        ...
        # Handle digital signature generation or invalidation
        if StatusApproval(new_status) == StatusApproval.APPROVED:
            try:
                from app.core.security_sign import CryptographicSigner
                validator = validated_by or old_booking.validated_by or "System"
                payload = {
                    "bid": old_booking.id,
                    "fac": old_booking.facility.name if old_booking.facility else "Facility",
                    "usr": old_booking.user.fullname if old_booking.user else "Civitas",
                    "dat": old_booking.date_of_booking.strftime("%Y-%m-%d"),
                    "tim": f"{old_booking.start_time.strftime('%H:%M')}-{old_booking.end_time.strftime('%H:%M')}",
                    "val": validator
                }
                signer = CryptographicSigner(private_key_hex=settings.SIGNING_PRIVATE_KEY)
                update_data["digital_signature"] = signer.sign_payload(payload)
            except Exception as e:
                logger.error("failed_to_generate_permit_signature", booking_id=old_booking.id, error=str(e))
        else:
            # Clear signature if booking is canceled or rejected
            update_data["digital_signature"] = None
    ...
    async def accept_handover(self, token: str) -> Booking:
        ...
        # Generate digital signature for the newly approved booking
        try:
            from app.core.security_sign import CryptographicSigner
            payload = {
                "bid": booking.id,
            ...
            signer = CryptographicSigner(private_key_hex=settings.SIGNING_PRIVATE_KEY)
            booking.digital_signature = signer.sign_payload(payload)
        except Exception as e:
            logger.error("failed_to_generate_handover_permit_signature", booking_id=booking.id, error=str(e))
